...
const DynamicInputForm = () => {
  ...
  useEffect(() => {
    ...
    var value = localStorage.getItem("selectedDropTime");
    if (value != null) {
  ...
  const handleSelectChange = (event) => {
    ...
    localStorage.setItem("dropTimingPrice", parseFloat(tp));
  };
...
function Step() {
  ...
  useEffect(() => {
    ...
    if (dropDate) {
      handleDateChange(dropDate);
    }
  }, []);

  //use effect
  useEffect(() => {
    //edit in local storage drop date

    localStorage.setItem("dropDate", selectedDate);
  }, [selectedDate]);
