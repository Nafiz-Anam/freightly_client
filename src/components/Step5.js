...
function Step() {
  ...
  const [pickupTimeSlotPrice, setPickupTimeSlotPrice] = useState(0);

  useEffect(() => {
    setSelectedOption(
      localStorage.getItem("selectedPickupTimeSlot") != null
        ? JSON.parse(localStorage.getItem("selectedPickupTimeSlot"))
        : ""
    );
  }, []);
  ...
  useEffect(() => {
    try {
      var price = pickupTimeSlotPrice.replace("€", "");
      localStorage.setItem("pickupTimeSlotPrice", parseInt(price));
    } catch (error) {
      // alert(error);
    }
  }, [pickupTimeSlotPrice]);
  ...
  const handleSelectChange = (event) => {
    ...
    localStorage.setItem("selectedPickupTimeSlot", JSON.stringify(value));
    var element = sheetData.find((element) => element[0] === value);
    setPickupTimeSlotPrice(element[1]);
    //alert(element[1]);
  };
